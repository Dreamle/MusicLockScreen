...
- (void)viewDidLoad {
    ...
    [self getLrcArrayWithIndex:0];
    self.customPlayer = [self customPlayerIndex:0];
...
//    }];
    ...
    [commandCenter.previousTrackCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent * _Nonnull event) {
        NSLog(@"上一首");
        [self getLrcArrayWithIndex:0];
        [self.customPlayer removeTimeObserver:_playerTimeObserver];
    ...
    [commandCenter.nextTrackCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent * _Nonnull event) {
        NSLog(@"下一首");
        [self getLrcArrayWithIndex:1];
        [self.customPlayer removeTimeObserver:_playerTimeObserver];
        _playerTimeObserver = nil;
        self.customPlayer = [self customPlayerIndex:1];
        [self.customPlayer play];
        [self playControl];
        return MPRemoteCommandHandlerStatusSuccess;
    }];
    ...
    [commandCenter.changePlaybackPositionCommand addTargetWithHandler:^MPRemoteCommandHandlerStatus(MPRemoteCommandEvent * _Nonnull event) {
        CMTime totlaTime = weakSelf.customPlayer.currentItem.duration;
        MPChangePlaybackPositionCommandEvent * playbackPositionEvent = (MPChangePlaybackPositionCommandEvent *)event;
        [weakSelf.customPlayer seekToTime:CMTimeMake(totlaTime.value*playbackPositionEvent.positionTime/CMTimeGetSeconds(totlaTime), totlaTime.timescale) completionHandler:^(BOOL finished) {
        }];
        return MPRemoteCommandHandlerStatusSuccess;
    }];
}
...
//获得歌词数组
- (void)getLrcArrayWithIndex:(NSInteger)index {
    wslAnalyzer *  analyzer = [[wslAnalyzer alloc] init];
    NSString *nameStr = self.songsArr[index];
    NSString * path = [[NSBundle mainBundle] pathForResource:nameStr ofType:@"txt"];
...
- (void)playControl{
    ...
    _playerTimeObserver = [weakSelf.customPlayer addPeriodicTimeObserverForInterval:CMTimeMake(0.1*30, 30) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        
        CGFloat currentTime = CMTimeGetSeconds(time);
        
        CMTime total = weakSelf.customPlayer.currentItem.duration;
        ...
        [self showLockScreenTotaltime:totalTime andCurrentTime:currentTime andRate:weakSelf.customPlayer.rate andLyricsPoster:isShowLyricsPoster lrcString:lrcString];
...
- (void)showLockScreenTotaltime:(float)totalTime andCurrentTime:(float)currentTime andRate:(NSInteger)rate andLyricsPoster:(BOOL)isShow lrcString:(NSString *)lrcStr {
    
    NSMutableDictionary * songDict = [[NSMutableDictionary alloc] init];
    //设置歌曲题目
    if (lrcStr.length) {
        [songDict setObject:lrcStr forKey:MPMediaItemPropertyTitle];
        //设置专辑名
        [songDict setObject:@"多幸运" forKey:MPMediaItemPropertyAlbumTitle];
    } else {
...
//- (AVPlayer *)customPlayer {
//
//    if (_customPlayer == nil) {
//        NSString * path = [[NSBundle mainBundle] pathForResource:SONGNAME ofType:@"mp3"];
//        _customPlayer = [[AVPlayer alloc] initWithURL:[NSURL fileURLWithPath:path]];
//    }
//    return _customPlayer;
//}


- (AVPlayer *)customPlayerIndex:(NSInteger)index {
    NSString *name = self.songsArr[index];
...
- (NSArray *)songsArr {
    if (!_songsArr) {
        self.songsArr = @[SONGNAME,SONGNAME2];
    }
    return _songsArr;
}
